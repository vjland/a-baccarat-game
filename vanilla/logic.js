...
export const generateBigRoad = (history) => {
  const matrix = Array.from({ length: 6 }, () => Array(500).fill(null));
  const path = [];
...
export const generateDerivedRoad = (bigRoadMatrix, path, offset) => {
  const derivedMatrix = Array.from({ length: 6 }, () => Array(500).fill(null));
  const validPath = path.filter(p => p.winner !== Winner.Tie);
  let dCol = 0, dRow = 0, dStreakStartCol = 0, lastColor = null;
