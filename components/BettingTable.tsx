...
export const BettingTable: React.FC<BettingTableProps> = ({ onBet, onClearBet, onClearAll, currentBets, balance, onDeal, showDeal }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-2 md:gap-4 p-2 md:p-4">
      {/* Betting Areas - Reduced Height by ~30% */}
      <div className="grid grid-cols-5 gap-2 h-16 md:h-28">
        <button 
      ...
      {/* Chip Selection & Balance - Condensed */}
      <div className="flex items-center justify-between gap-4 bg-black/50 p-2 md:p-4 rounded-2xl border border-white/10 shadow-2xl">
        <div className="flex items-center gap-4">
          <button onClick={onClearAll} className="px-3 py-1.5 rounded-lg bg-neutral-800 border border-white/10 text-neutral-400 text-[10px] font-black uppercase transition-all hover:bg-neutral-700">Recall</button>
          <div className="flex gap-2 md:gap-4">
            {CHIP_VALUES.map(val => (
              <button
                key={val}
                onClick={() => setSelectedChip(val)}
                className={`w-8 h-8 md:w-14 md:h-14 rounded-full border-4 flex items-center justify-center font-black text-xs md:text-lg transition-transform shadow-xl
          ...
          {showDeal && onDeal && (
            <button 
              onClick={onDeal}
              className="ml-2 bg-yellow-500 hover:bg-yellow-400 text-black px-6 py-1.5 md:px-12 md:py-3 rounded-xl font-black text-xs md:text-xl uppercase shadow-[0_0_20px_rgba(234,179,8,0.3)] transition-all active:scale-95 border-b-2 md:border-b-4 border-yellow-700 active:border-b-0 active:translate-y-1"
            >
              Deal
            </button>
          )}
        ...
        <div className="flex flex-col items-end">
          <span className="text-neutral-500 text-[10px] font-black uppercase tracking-widest leading-none">Bankroll</span>
          <span className="text-xl md:text-3xl font-black text-yellow-400 leading-none">${balance.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
