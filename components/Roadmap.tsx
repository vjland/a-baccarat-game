...
export const Roadmap: React.FC<RoadmapProps> = ({ history, shoeLength }) => {
  ...
  return (
    <div className="flex flex-col w-full bg-[#050a0e] p-0 gap-0 h-full min-h-0 overflow-hidden">
      {/* Top Section: Statistics Bar - Fonts and sizes reduced by 40% */}
      <div className="flex flex-col bg-black/80 border-b border-white/5">
        <div className="flex items-center gap-3 px-2 py-1 text-[10px] md:text-xs font-black text-neutral-400 uppercase tracking-widest">
          <span className="text-yellow-400">#{stats.total} HANDS</span>
          <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#1565c0]" /> {stats.p}</div>
          <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#c62828]" /> {stats.b}</div>
          <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#2e7d32]" /> {stats.t}</div>
          
          <div className="ml-auto flex items-center gap-2 text-[9px] md:text-[10px] text-neutral-500 lowercase font-bold">
